import sys, os, time, json, msvcrt, textwrap, hashlib, base64, re, traceback, importlib, errno, datetime

version = {"id": "1.0.0", "type": "experimental"}

computerUUID = os.popen("wmic csproduct get UUID").read()[40:76]
...
def setTitle():
    if version["type"] == "experimental":
        os.system("title PPPS Games Launcher v" + version["id"] + " (" + version["type"].title() + ")")
    else:
        os.system("title PPPS Games Launcher v" + version["id"])
...
def getData():
    ...
    try:
        ...
        response = json.loads(session.get("https://raw.githubusercontent.com/Pipatooa/ppps/master/authorisedComputers").text)
        
        authorisationLevel = -2
        for user in response:
            if user["uuid"] == computerUUID:
                authorisationLevel = user["allowed"]
    
        if authorisationLevel == -1:
            exit()
        
        response = session.get("https://raw.githubusercontent.com/Pipatooa/ppps/master/users")
        users = json.loads(response.text)
        
        response = session.get("https://raw.githubusercontent.com/Pipatooa/ppps/master/menuText")
        menuText = json.loads(response.text)
    
        response = session.get("https://raw.githubusercontent.com/Pipatooa/ppps/master/badWords")
        badWords = base64.b64decode(response.text).decode("ASCII").upper().split("\n")
        
        response = session.get("https://raw.githubusercontent.com/Pipatooa/ppps/master/gameData")
        gameData = []
        
        for game in json.loads(response.text):
            if game["visibility"] == 2:
                gameData.append(game)
            elif game["visibility"] == 1 and version["type"] == "experimental":
                gameData.append(game)
    except:
        raise
...
def runLoginMenu():
    ...
    loginMenu.objects["text"] = {"username": text(14, 18, 106, 18, "", 0), "password": text(14, 25, 106, 25, "", 0), "text": text(14, 32, 106, 32, "", 0)}
    ...
    while currentMenu == "login":
        ...
        if drawTime % 5 == 0:
            ...
            if authorisationLevel == -2:
                loginMenu.objects["text"]["text"].text = "-NOT AUTHORISED-                                                 (Press Esc for help screen)"
            elif authorisationLevel == 0:
                loginMenu.objects["text"]["text"].text = "-DISABLED-                                                       (Press Esc for help screen)"
            elif authorisationLevel == 1:
                loginMenu.objects["text"]["text"].text = "-AUTHORISED-                                                     (Press Esc for help screen)"
        ...
        if key:
            if key in (alphabet + [b" "]) and selected == 0 and len(username) < 20:
                ...
                password = password[:-1]
            elif key == 1072 and selected == 1:
                blinkTime = 0
                selected = 0
            elif key in [13, 1080] and selected == 0:
                blinkTime = 0
                selected = 1
                ...
                if authorisationLevel < 1 and len(username) > 0:
                    loginMenu.objects["errorText"][0].text = "This computer is not authorised"
                elif len(username) > 0:
                    for user in users:
...
def runHelpMenu():
    ...
    helpMenu = menu(120, 40, 4, 0, " ", {})
    helpMenu.objects["layout"] = [rectangle(4, 2, 115, 10, 6, None, " "), title(45, 4, "Help", 0, None, " "), rectangle(4, 13, 115, 37, 6, None, " ")]
    helpMenu.objects["text"] = [text(8, 15, 111, 35, menuText["help"], 0)]
    
    while currentMenu == "help":
        helpMenu.draw()
...
def runAuthorisationMenu():
    global currentMenu
    
    authorisationMenu = menu(120, 40, 4, 0, " ", {})
    authorisationMenu.objects["layout"] = [rectangle(4, 2, 115, 10, 6, None, " "), title(45, 4, "Help", 0, None, " "), rectangle(4, 13, 115, 37, 6, None, " ")]
    authorisationMenu.objects["boxes"] = [rectangle(12, 17, 107, 19, 7, None, " ")]
    authorisationMenu.objects["text"] = [text(14, 18, 106, 18, "COMPUTER UUID: " + computerUUID, 0), text(12, 23, 115, 35, menuText["authorisation"], 0)]
    
    while currentMenu == "authorisation":
        authorisationMenu.draw()
...
def runRegistrationMenu():
    ...
    while currentMenu == "registration":
        ...
        if key:
            ...
            elif key == 1072 and selected > 0:
                blinkTime = 0
                selected -= 1
            elif key in [13, 1080] and selected < 2:
                blinkTime = 0
                selected += 1
...
def runMainMenu():
    ...
    while currentMenu == "main":
        ...
        if key:
            if key in [1072, 119] and selected > 0:
                blinkTime = 0
                selected -= 1
                
                if selected < topResult:
                    topResult -= 1
            elif key in [1080, 115] and selected < len(gameData) - 1:
                blinkTime = 0
                selected += 1
...
def main():
    ...
    setTitle()
    setDisplaySize(30, 120, True)

    titleScreen = menu(120, 30, 4, 0, " ", [[rectangle(4, 2, 115, 27, 6, None, " "), title(44, 5, "PPPS", 0, None, " "), title(38, 12, "Games", 0, None, " "), title(27, 19, "Launcher", 0, None, " "), text(95, 26, 119, 26, "-By Bradley Hall-", 0)]])
    titleScreen.draw()
    time.sleep(1)
    ...
    while True:
        if currentMenu == "login":
            ...
            runHelpMenu()
        elif currentMenu == "authorisation":
            runAuthorisationMenu()
...
try:
    ...
    sys.path.append(os.getcwd() + "\ppps\imports")
    import requests, pyperclip
except:
    os.system("cls")
    errorMenu("Launcher was unable to import one or more required modules.\n\nPlease make sure that the folder \"ppps\" is up to date.")
    exit()
